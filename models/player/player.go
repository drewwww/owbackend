...
type Player struct {
	...
	BNetID	string	    `sql:"not null;unique" json:"bnetid"`  // Players Battle.net ID
...
func NewPlayer(bnetId string) (*Player, error) {
	player := &Player{BNetID: bnetId}
...
func (player *Player) SetExternalLinks() {
	...
	masteroverwatch := fmt.Sprintf(`masteroverwatch.com/profile/pc/us/%s`, player.BNetID)
	resp, err := helpers.HTTPClient.Get(masteroverwatch)
	if err == nil && resp.StatusCode == 200 {
		player.ExternalLinks["masteroverwatch"] = &masteroverwatch
	}
